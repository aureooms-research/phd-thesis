...
Indeed, each \(k\)-linear query of step \step{2} can be implemented as
\(O(k)\) arithmetic operations, so step \step{2} has complexity
\(O(\card{\net})\).
The construction of the simplex in step \step{3} must be handled carefully.
...
number of arithmetic operations and branching operations, step \step{3}
has complexity \(O(n\card{\net})\).
Finally, steps \step{1}, \step{4} and \step{5} are free since they do not involve the
input.
