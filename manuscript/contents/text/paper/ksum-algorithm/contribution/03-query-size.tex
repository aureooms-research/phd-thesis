...
\end{lemma}
\begin{proof}
Given an instance on $n$ numbers, we can sort them in $O(n\log n)$ time.
Partition
...
There exists a $o(n)$-linear decision tree of
depth $\tilde{O}(n^3)$ solving the \(k\)-SUM problem.
This decision tree can be implemented as a
$\tilde{O}(n^{\ceil{\frac{k}{2}}+8})$ Las Vegas word-RAM algorithm.
\end{corollary}
\begin{corollary}\label{cor:ne}
For any $\alpha$ such that $0<\alpha<1$,
there exists a $O(n^{1-\alpha})$-linear decision tree of
