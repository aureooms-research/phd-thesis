...
\(k\) even and \(O(n^{(k+1)/2})\) for \(k\) odd,
...
example the set
of \(n\) integers
$\{\,\frac{1-n}{2},\ldots,\frac{n-1}{2}\,\}$ (for odd $n$)
...
Using a trick due to Fredman~\cite{Fr76},
...
Chan and Lewenstein designed strongly subquadratic algorithms for a higher
dimensional variant of integer 3SUM with applications to jumbled
...
Chazelle, this indicates that increasing the size of the queries can yield
significant improvements on the depth of the minimal-height decision tree.

The results of Meyer auf der Heide and Meiser show that
if one is allowed to use $n$-linear queries instead, the complexity drops to a
polynomial in the input size whose exponent does not depend on $k$.
