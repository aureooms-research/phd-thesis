...
%
\(p(x) = \prod_{i=0}^{n-1} (x-r_i)\)
%
and computing
%
\begin{align*}
{\det(V)}^2 &= \mathrm{disc}_x(p(x)) \\
&= {(-1)}^{\binom{n}{2}} \mathrm{res}_x(p(x), \mathrm{diff}_x(p(x))) \\
&= {(-1)}^{\binom{n}{2}} \prod_{i=0}^{n-1} \mathrm{diff}_x(p(x))(r_i),
\end{align*}
%
...
\(\mathrm{diff}_x(p(x))\) at all \(r_i\)~\cite{St73,Ku73a,Ku73b,ASU75}.
