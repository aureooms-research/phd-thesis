...
%
If we want good lower bounds in the algebraic decision tree model and in the
algebraic computation tree model we need to resort to different proofs.
...
\begin{theorem}[name={Goodman and Pollack~\cite{GP86}, Alon~\cite{Al86}},label={thm:order-types:realizable}]
    There are  \(2^{\Theta(n \log n)}\) order types of point sets.
\end{theorem}
%
...
\begin{corollary}
    Any algebraic computation tree or bounded-tree algebraic decision tree that
    solves GPT has depth \(\Omega(n \log n)\).
\end{corollary}
...
\begin{proof}
    ...
    \end{displaymath}
    %
    Solving GPT on input \(q\) amounts to deciding whether \(q \in W\).
    %
    The set \(\mathbb{R}^{2n} - W\) has \(2^{\Theta(n \log n)}\) by
    \Cref{thm:order-types:realizable}.
\end{proof}
...
the
Petrovski\u{\i}-Ole\u{\i}nik-Thom-Milnor Theorem~\cite{Mi64,Th65,BPR06} that
bounds the number of connected components of algebraic varieties.
...
A substantial improvement can be made by using

Duality and constructing the dual arrangement
Construction of the arrangement~\cite[Theorem 24.4.1]{Hal04}
...
Pseudoline arrangements YES NOT IN TOOLBOX (MAYBE MOVE ARRANGEMENTS TO kSUM
SECTION?)
